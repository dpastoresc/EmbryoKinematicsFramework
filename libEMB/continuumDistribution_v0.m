...
%1-meanVel 2-meanVel_dif 3- stdVel 4-meanAngleRef 5-medianAngleRef 6-stdAngleRef 
%7-disMean 8-disStd 9-neigh 10-density 11-meanAngle 12- stdAngle 
%13- pow (all rings the same) %14-C2 (all rings the same)

function continuumDistribution_v0(Mov, posList, t, tag, Xave, dl, nbins, minSamples, X)
...
for i=1:ncells   
    ...
    samples=zeros(length(l));%number of cells found within the n*dl
    stats=zeros( nstats,length(l));
    
    c=1;%counter   
    isel=find(dAll>0 & dAll<=l_up);
    cells=
    ...
    for nl=1:length(l)
        ...
        if length(isel>0)
            ...
            NM=Mov(1:3, isel);
            angles=zeros(1,length(isel));
            anglesr=zeros(1,length(isel));
            selPos=posList(:,isel);
            dcount=1;      
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%% Absolute descriptors relative to the context
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %mean ave of the context
        [mAll iv]=removeOutlayers(mAll, 95, 5,-1);
        stats(1,nl)=mean(mAll);
        stats(2,nl)=median(mAll)-m;%cell against that mean
        stats(3,nl)=std(mAll);
        
        %mean direction of the context (use z-axis (1 0 0) as reference) 
        [anglesr iar]=removeOutlayers(anglesr, 95, 5, -1);
        stats(4,nl)=mean(anglesr(:));
        m_dev=mean(anglesr(:))-m_ar;
        stats(5,nl)=m_dev;
        stats(6,nl)=std(anglesr(:));
        ...
        angles=removeOutlayers(angles, 95, 5, -1);
...
save(['BigTable' tag '_t' num2str(t) '_dl' num2str(dl) '_bins' num2str(nbins) '_Xave' num2str(Xave)], 'BigTable');
save(['S2l' tag '_t' num2str(t) '_dl' num2str(dl) '_bins' num2str(nbins) '_Xave' num2str(Xave)], 'accpdecay');
save(['cells_t' num2str(t) '_dl' num2str(dl) '_bins' num2str(nbins) '_Xave' num2str(Xave)], 'cellstats');
