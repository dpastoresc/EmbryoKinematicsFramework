% /* --------------------------------------------------------------------------------------
%  * File:    generate_save_kinematics.m
%  * Date:    01/11/2018
...
stepsStored=size(cent,2)
if stepsStored<tfinal
   tfinal=stepsStored 
end
...
for t=tList
    ...
    [MovAux MeanMovAux TenseurGradAux NeighAux MovSlAux MeanMovSlAux RMovAux] = Create_VectorField_TensorField(datas,t,params,my_flags);
    Mov{t} = MovAux;
...
[Eall Rall Fall] = get_allTensors(TenseurGrad);%No error stuff
...
filename = [dataset '_RMov_T' int2str(params.tscale) '_X' int2str(params.Xscale) '_G' int2str(params.X) '.mat'];
save([savepath fsep dataset '_t' fsep filename],'RMov');
